/**
 * Tlumaczy katalog RAMEK (title, body_html, meta_title, meta_description) na
 * SV/CS/DA przez Shopify translationsRegister (Admin API) - ten sam
 * mechanizm co tlumaczKatalogSvCsDa.js (dla plakatow), ale ramki nie sa w
 * posters_inventory.json (osobna linia produktowa), wiec pobiera je wprost
 * z Shopify po tag:type_frame.
 *
 * Wznawialny: postep w tlumaczenia_ramki_progress.json (klucz: handle ->
 * {sv:true/false, cs:true/false, da:true/false}).
 *
 *   node scripts/tlumaczRamkiSvCsDa.js              -- wszystkie ramki
 *   node scripts/tlumaczRamkiSvCsDa.js --limit=5    -- pierwsze N (test)
 *   node scripts/tlumaczRamkiSvCsDa.js --handle=xyz -- jedna ramka
 */
...
const PROGRESS = path.join(ROOT, 'tlumaczenia_ramki_progress.json');
...
async function fetchAllFrameHandles() {
  const handles = [];
  let cursor = null;
  for (;;) {
...
async function translateWithOpenAI(title, bodyHtml, metaTitle, metaDescription) {
  const prompt = `Translate this Shopify product's fields into Swedish (sv), Czech (cs) and Danish (da). This is a poster/photo FRAME (not a poster) sold by a fine-art print shop — keep a natural, premium e-commerce tone. Preserve any HTML tags in body_html exactly, translate only the text inside them. Do not translate brand names or "REXIMPRIMIS". Do not translate material/color proper terms inconsistently — keep them natural in each language (e.g. "Wood", "Aluminum", "Black", "Oak"). Return ONLY valid JSON, no markdown fences, no commentary, in this exact shape:
...
(async () => {
  ...
  let list = onlyHandle ? [onlyHandle] : await fetchAllFrameHandles();
  if (limit) list = list.slice(0, limit);

  const progress = loadProgress();
  console.log(`Do przetlumaczenia: ${list.length} ramek (pomijam juz zrobione).`);
