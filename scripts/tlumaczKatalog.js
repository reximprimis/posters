 ...
 * WSZYSTKIE opublikowane jezyki sklepu naraz — PL, DE, SV, CS, DA — przez
 * Shopify translationsRegister (Admin API). Jedno wywolanie OpenAI zwraca
 * wszystkich 5 jezykow na raz (taniej i szybciej niz 5 osobnych wywolan).
 *
 * Zastepuje dwa osobne, wczesniej rownolegle istniejace skrypty
 * (tlumaczKatalogPlDe.js + tlumaczKatalogSvCsDa.js) — zebranie w jeden
 * plik wynika z tego, ze latwo bylo zapomniec odpalic oba, i dokladnie
 * to sie zdarzylo 2026-09-16 (patrz [[project-todo-auto-translations]]).
 * Stare pliki zostaja jako zachowana historia, ale NIE UZYWAC ich dalej —
 * ten skrypt jest jedynym zrodlem prawdy dla tlumaczen katalogu od teraz.
 *
 * NIE tlumaczy: handle (zepsulby URL-e), product_type (wewnetrzna taksonomia).
 *
 * Wznawialny: postep zapisywany w tlumaczenia_progress.json po kazdym
 * produkcie (klucz: handle -> {pl:true, de:true, sv:true, cs:true, da:true}).
 * Ponowne uruchomienie pomija produkty, ktore maja WSZYSTKIE jezyki gotowe.
 *
 *   node scripts/tlumaczKatalog.js              — caly katalog (ready+approved)
 *   node scripts/tlumaczKatalog.js --limit=5     — pierwsze N (do testu)
 *   node scripts/tlumaczKatalog.js --handle=xyz  — jeden produkt
 *   node scripts/tlumaczKatalog.js --handle=xyz --langs=sv,cs,da  — tylko wybrane jezyki
 */
...
const PROGRESS = path.join(ROOT, 'tlumaczenia_progress.json');
...
// Kolejnosc = kolejnosc opublikowanych jezykow w Shopify (shopLocales, poza
// "en" ktory jest primary/zrodlowy i nigdy nie jest tlumaczony).
const ALL_LOCALES = ['pl', 'de', 'sv', 'cs', 'da'];
const LOCALE_NAMES = { pl: 'Polish (pl)', de: 'German (de)', sv: 'Swedish (sv)', cs: 'Czech (cs)', da: 'Danish (da)' };
const LOCALES = langsArg ? langsArg.split(',').map((s) => s.trim()).filter((l) => ALL_LOCALES.includes(l)) : ALL_LOCALES;
...
async function translateWithOpenAI(title, bodyHtml, metaTitle, metaDescription, locales) {
  const localeNames = locales.map((l) => LOCALE_NAMES[l]).join(', ');
  const shape = locales
    .map((l) => `"${l}":{"title":"...","body_html":"...","meta_title":"...","meta_description":"..."}`)
    .join(',');

  const prompt = `Translate this Shopify product's fields into ALL of the following languages at once: ${localeNames}. Keep it natural, premium e-commerce tone (this is a fine-art poster print shop). Preserve any HTML tags in body_html exactly, translate only the text inside them. Do not translate brand names or "REXIMPRIMIS". Return ONLY valid JSON, no markdown fences, no commentary, in this exact shape:
{${shape}}
...
(async () => {
  ...
  if (!LOCALES.length) throw new Error('Brak poprawnych jezykow w --langs=');
  ...
  console.log(`Jezyki: ${LOCALES.join(', ')}`);
  console.log(`Do przetlumaczenia: ${list.length} produktow (pomijam juz kompletne).`);
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    const missing = LOCALES.filter((l) => !done[l]);
    if (missing.length === 0) { skip += 1; continue; }

    process.stdout.write(`[${i + 1}/${list.length}] ${handle} (${missing.join(',')})... `);
    try {
      ...
      const translated = await translateWithOpenAI(title, body, metaT, metaD, missing);

      const results = [];
      for (const locale of missing) {
        results.push(await registerLocale(tr.gid, locale, tr.byKey, translated[locale]));
      }
      ...
      if (errs.length) {
        ...
        progress[handle] = { ...done, ...Object.fromEntries(missing.map((l) => [l, true])) };
  ...
  console.log('');
  console.log(`Gotowe: ${ok} OK, ${skip} pominietych (juz kompletne), ${blad} bledow.`);
})();
