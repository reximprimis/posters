 ...
 * PL i DE przez Shopify translationsRegister (Admin API), zamiast Translate
 * & Adapt. Zrodlo tlumaczenia: Claude (Anthropic), jedno wywolanie na
 * produkt zwraca oba jezyki naraz.
 *
 ...
 * produkcie (klucz: handle -> {pl:true/false, de:true/false}). Ponowne
 * uruchomienie pomija juz zrobione.
 *
 *   node scripts/tlumaczKatalogPlDe.js              — caly katalog (ready+approved)
 *   node scripts/tlumaczKatalogPlDe.js --limit=5     — pierwsze N (do testu)
 *   node scripts/tlumaczKatalogPlDe.js --handle=xyz  — jeden produkt
 */
...
async function translateWithOpenAI(title, bodyHtml, metaTitle, metaDescription) {
  const prompt = `Translate this Shopify product's fields into Polish (pl) and German (de). Keep it natural, premium e-commerce tone (this is a fine-art poster print shop). Preserve any HTML tags in body_html exactly, translate only the text inside them. Do not translate brand names or "REXIMPRIMIS". Return ONLY valid JSON, no markdown fences, no commentary, in this exact shape:
{"pl":{"title":"...","body_html":"...","meta_title":"...","meta_description":"..."},"de":{"title":"...","body_html":"...","meta_title":"...","meta_description":"..."}}
...
(async () => {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (done && done.pl && done.de) { skip += 1; continue; }
    ...
    try {
      ...
      const both = await translateWithOpenAI(title, body, metaT, metaD);

      const rPl = await registerLocale(tr.gid, 'pl', tr.byKey, both.pl);
      const rDe = await registerLocale(tr.gid, 'de', tr.byKey, both.de);

      const errs = [...(rPl.userErrors || []), ...(rDe.userErrors || [])];
      if (errs.length) {
        console.log('BLEDY: ' + JSON.stringify(errs));
        blad += 1;
      } else {
        progress[handle] = { pl: true, de: true };
